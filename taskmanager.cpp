            ...
            for (auto itr = toDistributeTasks.begin(); itr != toDistributeTasks.end(); ++itr) {
                for (auto pos = itr->second.begin(); pos != itr->second.end();) {
                    ...
                    if (index >= nodes.size()) {
                        ...
                        int runTimes = 0;
                        try{
                            runTimes = stoi(task->getExtraParam("runTimes"))-1;
                        ...
                        else
                        {
                            ...
                            task->setExtraParam("runTimes", std::to_string(runTimes));
                            continue;
                        }
                    }
                    else {
                            ...
                            if (agv == nullptr) {
                                ...
                                AgvManager::getInstance()->foreachAgv(
                                    ...
                                    if (tempagv->getNowStation() != 0) {
                                        ...
                                        if (result_temp.size() > 0 && tempDis < minDis  || tempagv->getNowStation() == aimStation) {
                                            minDis = tempDis;
                                            bestAgv = tempagv;
                                            result = result_temp;
                                ...
                                if (bestAgv != NULL && minDis != DISTANCE_INFINITY) {
                                    ...
                                    MapManager::getInstance()->occuStation(aimStation, bestAgv);
                                    for (auto tline : result) {
                                        MapManager::getInstance()->addOccuLine(tline,bestAgv);
                                    }
									bestAgv->status = Agv::AGV_STATUS_TASKING;
                                ...
                                //指定车辆不空闲
                                if (agv->getTask() != task) {
                                    if (agv->status != Agv::AGV_STATUS_IDLE && agv->status != Agv::AGV_STATUS_WAITTING_PUT)
                                    {
                                        continue;
                                    }
                                }
                                ...
                                if (distance != DISTANCE_INFINITY && result.size() > 0 || agv->getNowStation() == aimStation) {
                                    //拿去执行//从未分配队列中拿出去
									 if(agv->status == Agv::AGV_STATUS_IDLE || agv->status == Agv::AGV_STATUS_WAITTING_PUT)
                                    {
                                        combined_logger->info("已分配AGV, 通知AGV有新任务... ");
...
void TaskManager::finishTask(AgvTaskPtr task)
{
	...
	if (agv != nullptr) {
		...
		//TODO:
        if(agv->status != Agv::AGV_STATUS_WAITTING_PUT)
        {
