...
/*
 *  floor name like this "floor_1, floor_2, floor_3", this function remove "floor_"
 */
int MapManager::getFloorNum(std::string floorName)
{
    if (floorName.length() == 0)
        return -1;

    size_t pos = floorName.find_first_of("_");
    if(pos == std::string::npos){
        combined_logger->error("MapManager::getFloorNum error, floorName must like floor_1");
        return -1;
    }

    std::string result = floorName.substr(pos+1);
...
int MapManager::getStationFloor(int station)
{
    ...
    for(auto f:floors){
        ...
        for(auto fp:fps){
            if(station == fp){
                //return f->getId();
                std::string floor_name = f->getName();
                return getFloorNum(floor_name);
